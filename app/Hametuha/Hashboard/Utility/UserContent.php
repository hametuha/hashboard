...
class UserContent extends Singleton {
	...
	protected function init() {
		add_action( 'init', [ $this, 'register_customs' ] );
		add_action( 'admin_init', [ $this, 'set_default_position' ] );
	}
	...
	public function register_customs() {
		...
		register_taxonomy( $this->taxonomy, [ 'user-content' ], $tax_args );
	}
	...
	public function set_default_position() {
		...
		foreach ( $positions as $slug => $position ) {
			...
			if ( $term ) {
				...
				// Create new term.
				wp_insert_term( $position, $this->taxonomy, [
					'slug' => $slug,
				] );
			}
		}
