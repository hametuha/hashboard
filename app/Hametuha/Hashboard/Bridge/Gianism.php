...
class Gianism extends Singleton {
	...
	protected function init() {
		if ( class_exists( 'Gianism\\Helper\\Option' ) && \Gianism\Helper\Option::get_instance()->is_enabled() ) {
			add_filter( 'hashboard_screen_children', [ $this, 'add_gianism_screen' ], 10, 2 );
			add_filter( 'hashboard_field_groups', [ $this, 'add_field_groups' ], 10, 4 );
			add_action( 'hashboard_before_fields_rendered', [ $this, 'account_notice' ], 10, 3 );
		}
	}
	...
	public function add_gianism_screen( $children, $slug ) {
		if ( 'account' == $slug ) {
			...
			add_action( 'hashboard_head', [ \Gianism\Bootstrap::get_instance(), 'enqueue_global_assets' ] );
		}
		return $children;
	}
	...
	public function add_field_groups( $fields, $user, $slug, $page ) {
		if ( 'account' == $slug && 'sns' == $page ) {
			ob_start();
			Hashboard::load_template( 'gianism.php', [ 'user' => $user ] );
